	...
	// continuously loop until progam exits
   	while(1) {
		...
		if (queue->qcount != 0) {
			...
			queue -> tail = (queue -> tail + 1) % (queue->qLength);
			queue -> qcount -= 1;
...
void init(long numberThreads, int qsize) {
	...
	queue -> fn = (void * (**)(void)) malloc(qsize * sizeof(void * (*)(void)));
	...
	for(int i = 0; i < numberThreads; i++){
		pthread_create(&(poolStruct -> pool[i]), NULL, emptyTask, (void *) (long) i); 
	}
...
void addTask(void * (fn)(void), int tasknum) {
	...
	queue -> head = (queue -> head + 1) % (queue->qLength);

	// increment the number of items in the queue 
	queue -> qcount += 1;//count or length?!!!

	// copy passed function pointer to the queue
	queue -> fn[tasknum % (queue->qLength)] = fn;
